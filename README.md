...
Practice, authored hints, arithmetic checks, atom/charge conservation checks, common-unit conversions, and graph exploration work without a model key. **Open-ended chat requires `OPENAI_API_KEY` and `OPENAI_MODEL` in the server process environment.** Use a model available to your API account that supports the Responses API and strict function tools. Do not paste credentials into chat or commit them. The app displays an explicit unconfigured state when either setting is absent; it does not simulate an AI reply.
